      ...
      <Section size="sm">
        {items.length === 0 ? (
          <div className="py-16 text-center rounded-3xl border border-border bg-secondary/40">
            ...
            <CTALink href="/gallery" className="mt-6 hover:bg-primary" withArrow>Browse gallery</CTALink>
          </div>
        ) : (
            ...
            <div className="space-y-4">
              {items.map((i) => (
                <div key={i.id} className="flex gap-4 rounded-2xl border border-border p-4">
                  <Link href={`/artwork/${i.id}`} className="shrink-0">
                    <img src={i.image} alt={i.title} className="h-24 w-24 md:h-28 md:w-28 object-cover rounded-xl bg-secondary" />
                  </Link>
                  <div className="flex-1 min-w-0">
                    <Link href={`/artwork/${i.id}`} className="font-semibold hover:underline underline-offset-4 truncate block">{i.title}</Link>
